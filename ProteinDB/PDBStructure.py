...
def getOneStrucByPath(pdb_path):
	...
	if(pdb_format==DB.PDB_FORMAT):
		if('AlphaFold'in pdb_path):
			line=pdb_path.split('.')[0]
			structure_id=line.split('\\')[-1]
			structure=p.get_structure(structure_id,pdb_path)
			return structure
		else:
			structure_id = pdb_path.split('.')[0][-4:]
			structure=p.get_structure(structure_id,pdb_path)
			return structure
	else:
		structure_id = pdb_path.split('.')[0][-4:]
...
def cleanStructure(struc):
    ...
    for chain in model:
        for residue in chain:
            ...
            if 'H_' in new[0]:
                res_to_remove.append(residue)
            if(residue.get_resname() not in residue_list or residue.id[0]!=' ' ): # mark not amino acid or Unknown residues #or  'H_' in (str(residue.id).split(','))[0]
                res_to_remove.append(residue)
            for atom in residue:
                if ( _hydrogen.match(atom.name)or d.match(atom.name)): # atom.altloc == "B" or
                    atom_to_remove.append(atom)
                if(residue.get_resname() in residue_atoms.keys() and atom.name not in residue_atoms[residue.get_resname()]):
...
def generateLenBystructure(struc,dir_path,file):#alphaFoldHuman_sasa_csv
	res_num=[r for r in struc[0].get_residues()]
	chains=PDBstructure(struc).chainLine()
	df=pd.DataFrame({'structure_id':[struc.id], 'chain_id':[chains],'length':[len(res_num)]},columns=['structure_id', 'chain_id','length'])
...
def getAlphaFoldHumanSingleChainStructures(structureid):
    chains=[c for c in alpha_struc.get_chains()]
    single_chains=[]
    if(len(chains)==1):
    	return True
    else:
...
if __name__ == '__main__':
	...
	if options.singlechain == True:
		#python PDBStructure.py --singlechain --pdb_code 1a8o
		pdb_code = '1a8o'
		path=test_directory+'/'+pdb_code+DB.PDB_FORMAT
		listp=[path]
		path = getSingleChainStructure(listp)
		structure=getOneStrucByPath(path[0])
		struc=cleanStructure(structure) 
		chains=[c for c in struc[0].get_chains()]
		if(len(chains)!=1):
			print('Fail')
	...
	if options.seq == True:
		...
		struc=cleanStructure(structure) 
		seqs = getStructureSequenceByChain(struc,chain)
		if(seqs!='TDSPVECMG'):
			print('Fail')

	# test getAlphaName
	if options.alphaname == True:
